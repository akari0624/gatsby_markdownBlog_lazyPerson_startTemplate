...
因為mdx檔是被`gatsby-mdx`當作`component`對待，所以在這個mdx raw file裡最下面可以看到我使用`gatsby`包裝過後的`graphQL`的`pageQuery`機制
``` javascript
// export name必須叫`pageQuery`
...
這個`graphQL`查詢的結果會被注入進這個`mdx`的props的`data`屬性底下
``` jsx
  <div>{props.data.site.siteMetadata.disqusShortname}</div>
```
而`siteMetadata`這組資料，在這個repo裡，是在`gatsby-config.js`裡被我們設定，
